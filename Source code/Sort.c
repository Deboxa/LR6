...
static int compare_two_elements_up(const data* value1, const data* value2) //сравнивание по возрастанию
{
	int s;
	if ((s = strcmp(value1->name, value2->name)) != 0)
		return s;
	if (value1->schoolnum != value2->schoolnum)
		return value1->schoolnum - value2->schoolnum;
	if ((s = strcmp(value1->region, value2->region)) != 0)
		return s;
	if ((s = strcmp(value1->subject, value2->subject)) != 0)
		return s;
	return value1->year - value2->year;

}

static int compare_two_elements_down(const data* value1, const data* value2) //сравнивание по убыванию
{
	int s;
	if ((s = strcmp(value2->name, value1->name)) != 0)
		return s;
	if (value2->schoolnum != value1->schoolnum)
		return value2->schoolnum - value1->schoolnum;
	if ((s = strcmp(value2->region, value1->region)) != 0)
		return s;
	if ((s = strcmp(value2->subject, value1->subject)) != 0)
		return s;
	return value2->year - value1->year;
}
...
static void fusion1(data* ptr, int left, int mid, int right, compare_ptr compare)
{
	...
	data* L = malloc(size1 * sizeof(data)); //выделение для левого подмассива
	if (L == NULL)
	{
		printf("Malloc error!\n");
		return;
	}
	data* R = malloc(size2 * sizeof(data)); //выделение для правого подмассива
	if (R == NULL)
	{
		printf("Malloc error!\n");
		return;
